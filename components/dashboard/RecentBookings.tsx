...
const statusStyles: Record<string, string> = {
  ...
  NO_SHOW: "bg-zinc-100 text-zinc-700 dark:bg-zinc-500/15 dark:text-zinc-300",
};
  ...
  return (
    <div id="bookings" className="rounded-[32px] border border-zinc-200/80 bg-white/95 p-6 shadow-sm shadow-zinc-200/30 backdrop-blur-xl dark:border-zinc-800/80 dark:bg-zinc-950/85 dark:shadow-zinc-950/15">
      <div className="flex items-center justify-between gap-4">
        ...
        <Link href="/bookings" className="rounded-2xl border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-100 dark:border-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-800">
          View all
        </Link>
      </div>
      ...
      <div className="mt-6 space-y-4">
        {recentBookings.length === 0 ? (
          <div className="py-8 text-center text-zinc-500 dark:text-zinc-400">No bookings yet</div>
        ) : (
          recentBookings.map((booking) => (
            <div key={booking.id} className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-zinc-200/75 bg-zinc-50 p-4 dark:border-zinc-800/70 dark:bg-zinc-900/90">
              <div className="space-y-1">
                <p className="text-sm font-semibold text-zinc-950 dark:text-zinc-100">{booking.customer.firstName} {booking.customer.lastName}</p>
                <p className="text-sm text-zinc-500 dark:text-zinc-400">{booking.bookingNumber}</p>
              </div>
              <div className="flex items-center gap-4">
                <span className={`rounded-full px-3 py-1 text-sm font-semibold ${statusStyles[booking.status]}`}>
                  {booking.status}
                </span>
                <span className="text-sm text-zinc-500 dark:text-zinc-400">{new Date(booking.eventDate).toLocaleDateString()}</span>
              </div>
            </div>
          ))
        )}
