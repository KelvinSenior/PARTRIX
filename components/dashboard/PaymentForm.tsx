...
export default function PaymentForm({ bookingId }: { bookingId?: string | null }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setError((err as Error).message || 'Could not record payment.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      <div className="flex gap-2">
        <input type="number" step="0.01" aria-label="Amount" className="flex-1 rounded-md border border-zinc-300 bg-white px-3 py-2 text-zinc-950 placeholder-zinc-500 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-50 dark:placeholder-zinc-400" value={amount} onChange={(e) => { setAmount(Number(e.target.value)); if (error) setError(null); }} placeholder="Amount" />
